...
Padronizar a organização de arquivos por classe ou serviço no código do
layerall, de modo que implementação, tipos, testes e mocks tenham lugar
previsível — e que múltiplas implementações de um mesmo contrato compartilhem
os testes. Precedente: o próprio layerall já usa isto (`hook-runner.ts` /
`.mock.ts` / `.test.ts`).
...
uma em memória), sem contar a implementação .mock.ts:
