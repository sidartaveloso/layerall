    ...
    const entry = map[url] ??
      map['*'] ?? {
        ...
        body: {
          ...
          latencyMs: 12,
          attempts: 1,
          providerReceipt: 'providerA:req_1:abc',
        },
      };
...
describe('Orchestrator SDK', () => {
  it('POSTs to /v1/operation/<op> with auth header', async () => {
    ...
    const res = await client.operation('create', {
      payload: { externalId: 'req_1', data: { x: 1 } },
    });
    ...
    const { fn, calls } = fakeFetch({
      'https://api.allx.com/v1/operation/send': {
        ...
        body: {
          ...
          latencyMs: 9000,
          attempts: 3,
          providerReceipt: 'providerB:req_2:z',
          error: { code: 'timeout', message: 'upstream timeout', provider: 'providerB' },
        },
      },
    });
    const client = new Orchestrator({
